...
class SetupScreen extends StatefulWidget {
  const SetupScreen({super.key, this.releasePageUriOverride});
...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _openReleasePage() async {
    ...
    try {
      final launched = await UpdateService.openReleasePage(
        releasePageUriOverride: widget.releasePageUriOverride,
      );
      if (!mounted) return;

      if (!launched) {
        final isConfigured = widget.releasePageUriOverride != null || UpdateService.isConfigured;
        final message = isConfigured ? '无法打开发布页，请稍后重试。' : '未配置发布页链接。';
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            onPressed: _isCheckingUpdate ? null : _openReleasePage,
            tooltip: '前往发布页',
